...
4. **Motion & icon tier** — `references/motion-icon-tiers.md`. Lottie is the sweet spot for expressive-but-themeable; GIF and video-loop are opt-in only, with the cost tradeoff stated to the user before use.
...
6. **Agent-state components** — presence indicator, thinking-vs-doing distinction, approval banner (must outrank everything else on screen), background-task notification, optional activity log, trustworthy error state, memory indicator. This is the differentiated core of the skill — do not skip or minify this section even under time pressure. Full detail in `references/agent-states.md`.
