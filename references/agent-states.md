...
## Persona roster (in scope) vs. multi-agent handoff (still out of scope) — these are different problems

**In scope, and worth doing well:** a single product offering several distinct, *named* agent personas the user picks between or invokes — the pattern shipped products in this space actually use (Block's Berd, for one, ships six: Berdy, Pushback, Choosey, Copycat, Tinker, Wildcard, each with a name, a role, and a one-line voice descriptor like "blunt, precise, no sugarcoating"). This is a voice-and-identity design problem, not an orchestration one: every persona in the roster shares one avatar state-machine tier and one token system, and differs by name, microcopy tone, and a single accent variation — not by separate design systems per persona. Maps to the schema's optional top-level `personas` array. The thing that makes a roster actually work is the same thing that makes any voice design work: if you could swap two personas' `vibe` lines and nothing would read wrong, the roster hasn't been designed yet, only labeled. Give each one a real, specific, differentiated voice the way `content-and-arc.md` asks for one agent's voice to be specific to its brief — just multiplied across a cast instead of a single character.

**Still out of scope for v1:** multi-agent *handoff/orchestration* UI — a supervisor agent delegating to sub-agents, a workflow-routing dashboard, hand-off transitions between agents mid-task, per-agent presence in a coordination view. That's a genuinely different, harder problem (workflow visualization, not voice design) and remains explicitly stubbed. If asked for this specifically, say so directly rather than improvising an unvalidated pattern, and point to the presence-indicator and activity-log tokens as the most likely extension points for a future version. Don't conflate the two scope calls — a persona roster is not a half-measure toward handoff UI, it's a complete, different, already-answerable request.
