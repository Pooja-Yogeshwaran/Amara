...
### Custom & thematic icon sets — e.g. "cute ghost icons for each tab"

The table above solves *generic* UI icons: send, close, settings, a chevron. It does not solve a **thematic or character request** — a mascot motif, "make the icons cute ghosts," a branded shape language specific to one product. That's a different problem with a different answer, and treating it like a library pick (silently substituting the nearest generic icon, or claiming some open-source set happens to have "cute ghosts") would be a real category error — no general-purpose OSI-licensed icon library reliably has the exact right character, and pretending otherwise either produces a mismatched result or an unverified license claim.
...
1. **Hand-authored original SVG** — simple, flat shapes (a rounded silhouette, dot eyes, no unnecessary detail) drawn to match `iconography.styleDescriptor`'s stroke width and corner style. Safe by construction — it's original artwork, not sourced from anywhere, so there's no license to get wrong. This is the default for a simple, iconic motif (a ghost, a leaf, a paw print) and what Amara should actually produce when asked. `customMotif.authorship: "hand-authored-original"`.
...
Path 1 is what makes "cute ghost icons for each tab" actually answerable rather than a request Amara has to decline or fudge — see `examples/minimalist-swiss/custom-icons/` for a worked instance (a 3-icon ghost tab-bar set, hand-authored as plain SVG paths, contrast-checked against the theme's tokens like any other icon).
