...
### Custom & thematic icon sets — any mascot or character motif, not one specific animal

The table above solves *generic* UI icons: send, close, settings, a chevron. It does not solve a **thematic or character request** — a mascot motif, "make the icons cute [X]" for any X (a ghost, a fox, a bird, a succulent, an invented blob-creature), a branded shape language specific to one product. That's a different problem with a different answer, and treating it like a library pick (silently substituting the nearest generic icon, or claiming some open-source set happens to have the exact right character) would be a real category error — no general-purpose OSI-licensed icon library reliably has the exact right subject, and pretending otherwise either produces a mismatched result or an unverified license claim.

Maps to `iconography.customMotif` (schema) rather than `setReference`, and is modeled like a miniature version of `avatar`'s state machine — same tier ladder (`static` → `micro-interaction` → `lottie` → `gif`, see `motion-icon-tiers.md`), because a custom icon set has the same expressiveness/cost tradeoff an avatar does. Three honest paths, in order of preference:

1. **Hand-authored original SVG** — simple, flat shapes drawn to match `iconography.styleDescriptor`'s stroke width and corner style, whatever the subject is. Safe by construction — it's original artwork, not sourced from anywhere, so there's no license to get wrong. This is the default for a simple, iconic motif and what Amara should actually produce when asked. `customMotif.authorship: "hand-authored-original"`.
2. **AI-generated, license reviewed independently** — for a motif too detailed or specific for quick hand-authoring. Uses the same caveat as the "AI graphic design tools" entry below: these are proprietary SaaS with their own generated-asset licensing terms, not covered by this repo's open-source-first posture, and must be reviewed per-tool before the output ships. Never assume a generated asset is "yours to use freely" by default. `customMotif.authorship: "ai-generated-review-license"`.
3. **Commissioned from a human illustrator** — outside Amara's own generation entirely, under whatever license is explicitly agreed. `customMotif.authorship: "commissioned"`.

Path 1 is what makes a request like this actually answerable rather than one Amara has to decline or fudge — see `examples/minimalist-swiss/custom-icons/` for one worked instance (a ghost tab-bar set, hand-authored as plain SVG paths, contrast-checked against the theme's tokens like any other icon). **That example is a demonstration of the mechanism for one past request, not a template or a fallback subject.** The next request that wants a custom motif and doesn't name one specifically should not default to a ghost because it's the only worked example on file — see `SKILL.md`'s Step 0 for the underlying rule (generic input still requires a specific, committed decision, not the nearest example) and design the actual subject asked for, or a specific original one if none was named, from scratch each time. If two unrelated requests for "a cute mascot icon set" would plausibly get the same subject and the same silhouette back, that's the failure this note exists to prevent, not a coincidence to shrug off.
